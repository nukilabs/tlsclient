...
func (d *ConnectDialer) DialContext(ctx context.Context, network, addr string) (net.Conn, error) {
	...
	case "http":
		...
		if err != nil {
			return nil, proxyError("Failed to dial proxy: " + err.Error())
		}
	...
	case "https":
		...
		if err := rawConn.HandshakeContext(ctx); err != nil {
			return nil, proxyError("Failed to handshake with proxy: " + err.Error())
		}
		...
		default:
			return nil, proxyError("Unsupported negotiated protocol: " + state.NegotiatedProtocol)
		}
	default:
		return nil, proxyError("Unsupported proxy scheme: " + d.ProxyURL.Scheme)
	}
}
...
func (d *ConnectDialer) connectHttp1(req *http.Request, rawConn net.Conn) (net.Conn, error) {
	...
	if err := rawConn.SetDeadline(deadline); err != nil {
		rawConn.Close()
		return nil, proxyError("Failed to set deadline: " + err.Error())
	}

	if err := req.Write(rawConn); err != nil {
		rawConn.Close()
		return nil, proxyError("Failed to write proxy request: " + err.Error())
	}

	res, err := http.ReadResponse(bufio.NewReader(rawConn), req)
	if err != nil {
		rawConn.Close()
		return nil, proxyError("Failed to read proxy response: " + err.Error())
	}

	if res.StatusCode != http.StatusOK {
		rawConn.Close()
		return nil, proxyError("Proxy responded with non 200 code: " + res.Status)
	}
...
func (d *ConnectDialer) connectHttp2(req *http.Request, rawConn net.Conn, h2clientConn *http2.ClientConn) (net.Conn, error) {
	...
	if err != nil {
		rawConn.Close()
		return nil, proxyError("Failed to round trip: " + err.Error())
	}
